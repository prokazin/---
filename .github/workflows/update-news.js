// ===== update-news.js — запускается в GitHub Actions =====
const fs = require('fs');
...
async function main() {
    ...
    const mainNews = newNews.filter(item => !isAltcoinNews(item.title)).slice(0, 3);
    const altNews = newNews.filter(item => isAltcoinNews(item.title)).slice(0, 3);
